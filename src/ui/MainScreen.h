...
public:
  MainScreen() { store = new Store({133, 45}); }
  void printUI() {
    ...
    // Bag
    BagModal bagModal;
    auto bag_modal_component =
        bagModal.modalUI([&] { bag_modal_shown = false; });
    ...
    container = ResizableSplitBottom(player_row, container,
    &player_row_size); container |= Modal(bag_modal_component,
    &bag_modal_shown); container |= Modal(store_modal_component,
    &store_modal_shown);
