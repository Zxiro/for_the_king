...
public:
  Player(const Position &_position, const std::string &_display)
      : Creature(_position, _display) {}
  ...
  //在移動前，根據Stat::Speed決定可以投擲多少個骰子，骰子的數量就是
  // MaxMovementPoint
  int getMaxMovementPoint() const { return std::floor(getSpeed() / 10); }
  ...
  // 取得角色從骰完骰子後移動了幾步
  int getMovementCount() { return this->currentMovementCount; }
  ...
  void removeWeapon() { this->weapon = nullptr; }

  void removeArmor() { this->armor = nullptr; }

  void removeAccessory() { this->accessory = nullptr; }

  Weapon *getWeapon() { return this->weapon.get(); }
  ...
  Accessory *getAccessory() { return this->accessory.get(); }
  // items function

  void useItem(std::shared_ptr<Item> item,
               const Position &position = Position{}, int playerIndex = -1) {
    if (!item)
      return;
  ...
  // getter setter
  double getVitality() const override { return vitality; }
  ...
  double getPDefense() const override {
    ...
    int accessoryPDefense =
        EffectUtil<Accessory>::getPDefense(this->accessory.get());
    int res = 0;
  ...
  bool canTeleport() const override { return true; }

  void setVitality(int vit) { vitality = vit; }
  ...
  void setAttack(int atk) { pAttack = atk; }

  void setPDefense(int def) { pDefense = def; }

  void setMDefense(int def) { mDefense = def; }
...
private:
  ...
  double getRollDiceChance() const { return getSpeed() / 10; }
